...
def save_dialogue_act_labels(dialogue_act_labels, emotion_data, meeting_id, cache_dir='./data', output_filename="dialogue.txt"):
    """
    Save the dialogue act labels combined with emotion text data to a text file and store the results in a JSON file.
    ...
    emotions_safe = {
        "positive": int(float(emotions_data['positive_percentage'].replace('%', ''))),
        "negative": int(float(emotions_data['negative_percentage'].replace('%', ''))),
        "neutral": int(float(emotions_data['neutral_percentage'].replace('%', '')))
    }

    kpis_safe = {
        "engagement": float(emotions_data['kpis']['engagement']),
        "alignment": float(emotions_data['kpis']['alignment']),
        "agency": float(emotions_data['kpis']['agency']),
        "stress": float(emotions_data['kpis']['stress']),
        "burnout": float(emotions_data['kpis']['burnout'])
    }
    ...
    word_count_data = {k: {str(ks): float(vs) for ks, vs in v.items()} for k, v in word_count_data.items()}
    ...
    raw_dimensions = {key: float(value) for key, value in zip(r_keys, radar_chart_list)}
    ...
    {
        'positive': emotions_safe['positive'],
        'negative': emotions_safe['negative'],
        'neutral': emotions_safe['neutral']
    },
    {
        'engagement': kpis_safe['engagement'],
        'alignment': kpis_safe['alignment'],
        'agency': kpis_safe['agency'],
        'stress': kpis_safe['stress'],
        'burnout': kpis_safe['burnout']
    })
    ...
    json_data = {
        ...
        "emotions_safe": emotions_safe,
        "kpis_safe": kpis_safe
    }

    json_path = os.path.join(cache_dir, "output.json")
